...
if (ISWP){
    ...
    if (profile){
        profile.setAttribute("href", "weixin://profile/wxid_2117831151212");
    }
}
var tid = "";
var aid = "";
...
var title     = trim('Ring：蓝牙手势操控指环'      .replace(/&lt;/g, '<').replace(/&gt;/g, '>'));
var sourceurl = trim('http://www.leiphone.com/ring.html#rd'.replace(/&lt;/g, '<').replace(/&gt;/g, '>'));
...
function report_article(){
    ...
    location.href = '/mp/readtemplate?t=wxm-appmsg-inform&__biz=MTM2ODM0ODYyMQ==&info=' + encodeURIComponent(query) + "#wechat_redirect";
}
...
(function(){
    ...
    function onBridgeReady() {
        ...
        var appId  = '',
        imgUrl = "http://mmbiz.qpic.cn/mmbiz/YjhmbbkdV6uib7ZUVa7Ly8cgNic8aPQJ3gvibkcnmthicaROxwZvX2yqNxAKOX86Xy3ThKJFLwRvJSSOvCaIhSU48w/0",
        link   = "http://mp.weixin.qq.com/s?__biz=MTM2ODM0ODYyMQ==&mid=200102302&idx=2&sn=89b8759a714c22cc16d11d2bbd0dad11#rd",
        title  = htmlDecode("Ring：蓝牙手势操控指环"),
        desc   = htmlDecode("随着可穿戴式设备不断的发展和创新，穿戴式设备已经能够好好的融入我们的生活，给我们的生活提供便利和乐趣。今天介"),
        fakeid = "OTAxMDgxMTU=";
        desc   = desc || link;  
...
(function(){
    var timeout = null;
    var val = 0;
    var url = "http://mp.weixin.qq.com/s?__biz=MTM2ODM0ODYyMQ==&mid=200102302&idx=2&sn=89b8759a714c22cc16d11d2bbd0dad11#rd".split('?').pop();
    var key = hash(url);
    /*
    ...
    for(var i=0, l=imageEls.length; i <l; i++) {
        ...
        for(var j = 0; j < entities.length; j++) {
            ...
            if (entity.name == src) {
                ajax({  
                    type : "POST",
                    url: "/mp/appmsgpicreport?__biz=MTM2ODM0ODYyMQ==" + "&uin=" + uin + "&key=" + key + "#wechat_redirect",
                    data: obj2query({
                        rnd: Math.random(),
                        uin: uin,
