...
class reply_admin_lib
{
	...
	function _keyword(){
		...
		redirect('reply/admin/reply/keylist','location');
	}
	...
	function _menu(){
		...
		redirect('reply/admin/reply/menu','location');
	}
	...
	function wxmennuData($type='R',$newdata=array()){
		$file=BASEPATH."cache/wx_diymenu";
		if($type=='R'){
			$data=@include($file);
	...
	function _msg(){
		...
		redirect('reply/admin/reply/msg/'.$type,'location');
	}
}