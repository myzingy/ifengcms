...
<?php print form_open_multipart('vote_zh/admin/vote/bindsource',array('class'=>'horizontal','id'=>'bindsource'))?>
<div class="buttons">
...
<div class="buttons">
	<a href="<?php print  site_url('vote_zh/admin/vote/update/'.$vid)?>">
    <?php print  $this->bep_assets->icon('cog');?>
    	修改投票配置
    </a>            
	<a href="<?php print  site_url('vote_zh/admin/vote/votedata/'.$vid)?>">
	...
	pageinit=function(){
		...
		$('.cleanSource').click(function(){
			if(confirm('确定要解除此绑定吗？')){
				...
				$.get('<?php print site_url('vote_zh/admin/vote/unbindsource/'.$vid)?>/'+source_id,function(){
					location.reload();
				});
			}
		});
		...
		var getData=function(source_id){
			var url='<?php print site_url('vote_zh/admin/vote/startSource/'.$vid)?>/'+source_id+'/page/'+page;
			$.ajax({
				url:url,
				dataType:'json',
				success:function(json){
