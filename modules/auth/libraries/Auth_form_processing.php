...
class Auth_form_processing
{
	 ...
	 */
	function login_form($container,$ajax='')
	{
		...
		{
			if($ajax){
				$arr=array('status'=>10000,'error'=>$this->CI->validation->_error_array[0]);
				die(json_encode($arr));
			}	
			// Output any errors
	...
	function _login($ajax='')
	{
		...
		if ( $result['valid'] )
		{
			...
			if ( $user->active == 0 )
			{
				// NEEDS ACTIVATION
				if($ajax){
					$arr=array('status'=>10000,'error'=>$this->CI->lang->line('userlib_account_unactivated'));
					die(json_encode($arr));
				}
		...
		else
		{
			...
			if($ajax){
				$arr=array('status'=>10000,'error'=>$this->CI->lang->line('userlib_login_failed'));
				die(json_encode($arr));
			}
			flashMsg('error',$this->CI->lang->line('userlib_login_failed'));
		}
		if($ajax){
			die('{"status":10000,"error":"'.$this->CI->lang->line('userlib_login_failed').'"}');
		}
		redirect('auth/login','location');
	}
	 ...
	 * @access private
	 */
	function _register($ajax='')
	{
		// Build
		$data['users']['username'] = $this->CI->input->post('username');
		$data['users']['email'] = $this->CI->input->post('username').'@139.com';
		...
		if(!checkcode($data['users']['username'],$code)){
			if($ajax){
				$arr=array('status'=>10000,'error'=>'手机验证码错误');
				die(json_encode($arr));
			}
			flashMsg('error','手机验证码错误');
		...
		if ($this->CI->db->trans_status() === FALSE)
		{
			...
			if($ajax){
				$arr=array('status'=>10000,'error'=>$this->CI->lang->line('userlib_registration_failed'));
				die(json_encode($arr));
			}
	 ...
	 * @param string $container View file container
	 */
	function register_form($container,$ajax='')
	{
		if( ! $this->CI->preference->item('allow_user_registration'))
		{
			// If registration is not allowed
			if($ajax){
				$arr=array('status'=>10000,'error'=>$this->CI->lang->line('userlib_registration_denied'));
				die(json_encode($arr));
			}
		...
		$rules['code'] = 'trim|required|max_length[6]';
	...
	function sendcode(){
		$this->CI->load->module_helper('phonecode','phonecode');
		$fields['username'] = '手机号码';
		$rules['username'] = 'trim|required|min_length[11]|max_length[11]|spare_username';
		$this->CI->validation->set_rules($rules);
		if ( $this->CI->validation->run() === FALSE )
		{
			$arr=array('status'=>10000,'error'=>'手机号码错误');
			die(json_encode($arr));
		}
		$phone = $this->CI->input->post('username');
		sendcode($phone,1);
		die('{"status":0}');
	}
}
