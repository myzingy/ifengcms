...
<div class="buttons">                
	<a href="<?php print  site_url('article/admin/article/form/sscgame')?>">
    <?php print  $this->bep_assets->icon('add');?>
    	添加游戏
    </a>
</div><br/><br/>
<?php print form_open('article/admin/article/delete/sscgame')?>
<table class="data_grid" cellspacing="0">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="middle"><a href="<?php print site_url('article/admin/article/form/sscgame/'.$row['id'])?>"><?php print $this->bep_assets->icon('pencil');?></a></td>
	...
	function switchStatus(id,title,status){
		if(confirm('确定要将'+title+(status==0?'下架':'上架')+'吗？')){
			$.get('<?php print site_url('article/admin/article/switchStatus/');?>/'+id+'/'+Math.abs(status-1),function(){
				location.href='<?php print site_url('article/admin/article/index/sscgame');?>';
			});
		}
	}
</script>