...
<div class="buttons">                
	<a href="<?php print  site_url('article/admin/article/form/top')?>">
    <?php print  $this->bep_assets->icon('add');?>
    	添加推荐
    </a>
</div><br/><br/>

<?php print form_open('article/admin/article/delete/top')?>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td><?php 
				...
				foreach($pos as $key):
				?>
            	<?php print $position[$key]?>&nbsp;
            	<?php endforeach;?>
            </td>
            <td><?php print date("Y-m-d H:i:s",$row['addtime'])?></td>
            <td><?php print $this->bep_assets->icon($active);?></td>
            <td class="middle"><a href="<?php print site_url('article/admin/article/form/top/'.$row['id'])?>"><?php print $this->bep_assets->icon('pencil');?></a></td>
