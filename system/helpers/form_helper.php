	...
	function rules_ci2js($rules,$fields){
		...
		foreach($rules as $field=>$r){
			...
			foreach($_arr as $v){
				...
				if(preg_match("/([minax]{3})_length\[([0-9]+)\]/",$v,$tmp)){
					$__arr[]="{$tmp[1]}length:{$tmp[2]}";
					$__arr_message["{$tmp[1]}length"]=langx("{$tmp[1]}_length",array($fields[$field],$tmp[2]));
				}
				//if(ereg("([minax]{3})_number\[([0-9]+)\]",$v,$tmp)){
				if(preg_match("/([minax]{3})_number\[([0-9\.]+)\]/",$v,$tmp)){
					$__arr[]="{$tmp[1]}:{$tmp[2]}";
					$__arr_message["{$tmp[1]}"]=langx("{$tmp[1]}_number",array($fields[$field],$tmp[2]));
				}
				//if(ereg("matches\[(.+)\]",$v,$tmp)){
				if(preg_match("/matches\[(.+)\]/",$v,$tmp)){
					$__arr[]="equalTo:'#{$tmp[1]}'";
					$__arr_message["equalTo"]=langx("matches",array($fields[$field],langx($tmp[1])));
				}
				if($v=="required"){
					$__arr[]="{$v}:true";
					$__arr_message["required"]=langx($v,$fields[$field]);
				}
				if($v=="numeric"){
					$__arr[]="number:true";
					$__arr_message["number"]=langx($v,$fields[$field]);
				}
				if($v=="integer"){
					$__arr[]="digits:true";
					$__arr_message["digits"]=langx($v,$fields[$field]);
				}
				if($v=="valid_email"){
					$__arr[]="email:true";
					$__arr_message["email"]=langx($v,$fields[$field]);
				}
				if($v=="prep_url"){
					$__arr[]="url:true";
					$__arr_message["url"]=langx($v,$fields[$field]);
				}
			}
			if($__arr){
